      ...
      <div className="flex flex-col w-fit">
        ...
        <div className={`${view === "grid" ? "grid grid-cols-1 self-center md:gap-4 xl:grid-cols-3 2xl:grid-cols-3 lg:grid-cols-2 sm:grid-cols-2" : ""}`}>
          {fields.length ? fields.map((card, i) => (
            !card.archived && <Card key={i} id={card.id} titulo={card.title} descripcion={card.description} hectareas={card.hectares} lotes={card.lots} ubicacion={card.location} terrain={card.terrain} price={card.price} images={card.images} boolean={boolean} sold={card.sold} setBoolean={setBoolean} archived={card.archived} />
          ))
            :
