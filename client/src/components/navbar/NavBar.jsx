...
export default function NavBar() {
  ...
  return (
    <div className='relative flex justify-center'>
      ...
      <nav className={`z-[60] ${pathname === "/" ? "absolute bg-transparent" : ""} top-0 w-full bg-[#276163]`}>
          ...
          <div className={`animate-movimiento max-[768px]:absolute max-[768px]:top-[125px] max-[768px]:lef-0 max-[768px]:right-0 items-center justify-center ${isOpen ? "" : "hidden"} md:h-fit w-full md:flex z-30 `} >
            <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:gap-8 md:mt-0 md:border-0 md:bg-transparent md:items-center">
              ...
              <li className='hidden md:flex h-fit'>
                <button aria-label="dropdown" id="dropdownNavbarLink" data-dropdown-toggle="dropdownNavbar" className="flex items-center justify-between w-full py-2 pl-3 pr-4 text-white rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 hover:text-black md:p-0 md:w-auto">
                  Comprar
                ...
                <div id="dropdownNavbar" className="z-10 hidden font-normal bg-white divide-y divide-gray-100 rounded-lg shadow w-44">
                  <ul className="py-2 text-sm text-gray-700" aria-labelledby="dropdownLargeButton">
                    <li>
                      <a aria-label="fields" href="#campos" onClick={() => history.push("/")} className="text-lg block px-4 py-2 hover:bg-gray-100">Campos</a>
                    </li>
                    <li>
                      <a aria-label="apartments" href="#campos" onClick={() => history.push("/")} className="text-lg block px-4 py-2 hover:bg-gray-100">Departamentos</a>
                    </li>
                  </ul>
                </div>
              </li>
              <li className='h-fit md:hidden'>
                <a aria-label="fields" href="#campos" onClick={() => { setIsOpen(!isOpen); history.push("/") }} className=" block py-2 pl-3 pr-4 text-black md:text-white rounded text-lg hover:bg-gray-100 md:hover:bg-transparent md:hover:text-black md:p-0">
                  Campos
                </a>
              </li>
              <li className='h-fit md:hidden'>
                <a aria-label="apartments" href="#campos" onClick={() => { setIsOpen(!isOpen); history.push("/") }} className="block py-2 pl-3 pr-4 text-black md:text-white rounded text-lg hover:bg-gray-100 md:hover:bg-transparent md:hover:text-black md:p-0">
                  Departamentos
                </a>
              </li>
              <li className="hidden md:flex h-fit">
              ...
              <li className='h-fit'>
                <Link to="/Mapa" onClick={() => setIsOpen(!isOpen)} className={`block py-2 pl-3 pr-4 text-black rounded text-lg hover:bg-gray-100 md:hover:bg-transparent md:hover:text-black md:p-0 ${pathname === "/Nosotros" ? "md:text-black" : "md:text-white"}`}>
                  Mapa de propiedades
                </Link>
              </li>
              <li className='h-fit'>
              ...
              <li className='h-fit md:hidden'>
                <a aria-label="call" href="tel:2473509269" onClick={() => setIsOpen(!isOpen)} target="_blank" className="md:hidden block py-2 pl-3 pr-4 text-black md:text-white  rounded text-lg hover:bg-gray-100 md:hover:bg-transparent md:hover:text-black md:p-0">
                  Llamanos
                </a>
              </li>
            </ul>
